...
// Lazy-loaded semantic-release modules (ESM)
let semanticGetConfig;
...
async function getConfigSemantic({ cwd, env, stdout, stderr, logger }, options) {
	try {
		// Lazy-load semantic-release ESM module
		if (!semanticGetConfig) {
			semanticGetConfig = (await import("semantic-release/lib/get-config.js")).default;
		}
